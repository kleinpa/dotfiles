...
indentation."
  (let ((normal-indent (current-column)))
	...
	(cond ((or (eq method 'defun)
	      ...
	      ((null method)
	       (scheme-nice-indent state indent-point normal-indent))
	      ((integerp method)
...
(defun scheme-normal-indent (state indent-point normal-indent) normal-indent)
...
(let ((symbols
  ...
  (dolist (symbol symbols)
    (put symbol 'scheme-indent-function 'scheme-normal-indent)))

(defun scheme-nice-indent (state indent-point normal-indent)
  (let ((containing-form-start (elt state 1))
	body-indent eol)
    (goto-char containing-form-start)
    (setq body-indent (+ (current-column) lisp-body-indent))
    (end-of-line)
    (setq eol (point))
    (goto-char containing-form-start)
    (forward-char 1)
    (forward-sexp 1)
    (parse-partial-sexp (point) eol 1 t)
    (if (< (point) eol)
	body-indent
      normal-indent)))
...
(put 'dynamic-wind 'scheme-indent-function 0) ; r5rs?

